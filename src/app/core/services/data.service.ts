...
import { Player, Jornada } from '../models';
...
export class DataService {
  ...
  private calculatePlayers(players: Player[]): void {
    ...
    players.forEach((player) => {
      ...
      player.points.filter((value): value is number => value !== null).forEach((score, index) => {
    ...
    Object.keys(calculateJornadas).forEach((jornada) => {
      calculateJornadas[jornada].sort((a, b) => 
        cmp(
          (a.score_jornada as unknown as number | string) > (b.score_jornada as unknown as number | string) ? -1 : 
          (a.score_jornada as unknown as number | string) < (b.score_jornada as unknown as number | string) ? 1 : 0,
          (b.score_jornada as unknown as number | string) > (a.score_jornada as unknown as number | string) ? -1 : 
          (b.score_jornada as unknown as number | string) < (a.score_jornada as unknown as number | string) ? 1 : 0
        ) || cmp(a.team, b.team)
      );
      ...
      calculateJornadas[jornada].forEach((team, i) => {
        ...
        if (i > 0 && team.score_jornada === calculateJornadas[jornada][i - 1].score_jornada) {
          team.position_jornada = calculateJornadas[jornada][i - 1].position_jornada;
        } else {
        ...
        const validPoints = player.points.filter((v): v is number => v !== null);
    ...
    players.forEach((player) => {
      player.top_jornada = player.positions_jornada?.filter((v) => v === 1).length ?? 0;
      player.top_clasificacion = player.positions_general?.filter((v) => v === 1).length ?? 0;

      player.positions_general_differences = player.positions_general?.map((value, index) => 
        index === 0 ? 0 : (player.positions_general?.[index - 1] ?? 0) - value
      ) ?? [];
      
      const diffs = player.positions_general_differences.filter((v): v is number => !isNaN(v));
      player.positions_general_differences_max = diffs.length === 0 ? 0 : Math.max(...diffs);
      player.positions_general_differences_min = diffs.length === 0 ? 0 : Math.abs(Math.min(...diffs));
    });
  }
  ...
  getJornadas(): Record<string, Jornada> {
    const players = this.playersData();
    const cmp = (x: number | string, y: number | string): number => {
      if (typeof x === 'string' && typeof y === 'string') {
        return x.localeCompare(y);
    ...
    const calculateJornadas: Record<string, any> = {};
    ...
    players.forEach((player) => {
      ...
      player.points.filter((v): v is number => v !== null).forEach((score, index) => {
        player.score_general! += score;
        player.score_jornada!.push(player.score_general!);

        const key = `jornada_${index + 1}`;
        if (!calculateJornadas[key]) {
          calculateJornadas[key] = [];
        }
    ...
    Object.keys(calculateJornadas).forEach((jornada) => {
      ...
      calculateJornadas[jornada].sort((a: any, b: any) => {
      ...
      calculateJornadas[jornada].score_worst = { name: '', team: '', score: 1000 };
      calculateJornadas[jornada].score_average = 0;
      calculateJornadas[jornada].name = `JORNADA ${countJornada}`;

      const pointsAverage: number[] = [];
      calculateJornadas[jornada].forEach((team: any, i: number) => {
        if (i > 0 && team.score_jornada === calculateJornadas[jornada][i - 1].score_jornada) {
          team.position_jornada = calculateJornadas[jornada][i - 1].position_jornada;
        } else {
          team.position_jornada = i + 1;
      ...
      calculateJornadas[jornada].sort((a: any, b: any) => {
        ...
        if (scoreDiff !== 0) return scoreDiff;
        const bestDiff = cmp(b.score_best - b.score_worst, a.score_best - a.score_worst);
        return bestDiff;
      });

      calculateJornadas[jornada].forEach((team: any, i: number) => {
        if (i > 0 && team.score_general === calculateJornadas[jornada][i - 1].score_general) {
          team.position_general = calculateJornadas[jornada][i - 1].position_general;
        } else {
        ...
        if ((countJornada + 1) === lastJornada) {
          penultima[i] = { id: team.id, team: team.team, position: team.position_general };
        }
        if (countJornada === lastJornada && lastJornada > 1) {
          ...
          if (player2) {
            ...
            team.updown_num = Math.abs(player2.position - team.position_general);
          }
        } else {
          team.updown = 'fas fa-equals';
          team.updown_num = 0;
      ...
      if (!calculateJornadas['score_best']) {
        calculateJornadas['score_best'] = { name: '', team: '', score: 0 };
      }
      if (!calculateJornadas['score_worst']) {
        calculateJornadas['score_worst'] = { name: '', team: '', score: 1000 };
      }
      if (!calculateJornadas['score_average']) {
        calculateJornadas['score_average'] = 0;
      }

      const jornadaBest = calculateJornadas[jornada].score_best;
      if (calculateJornadas['score_best'].score < jornadaBest.score) {
        calculateJornadas['score_best'] = { ...jornadaBest };
      }

      const jornadaWorst = calculateJornadas[jornada].score_worst;
      if (calculateJornadas['score_worst'].score > jornadaWorst.score) {
        calculateJornadas['score_worst'] = { ...jornadaWorst };
      }
    });
