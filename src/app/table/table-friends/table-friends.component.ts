    ...
    if (Object.values(this.tables).length > 0) {
      ...
      this.tablePlayers.forEach((player, index) => {
        if (this.sportFriends.includes(player['id'])) {
          const myplayer = this.general_table.filter((e: any) => e.id === player.id);
      ...
      this.general_table = this.general_table.filter((player: any) => this.sportFriends.includes(player['id']));
      ...
      this.general_table.forEach((element: any, index: number) => {
