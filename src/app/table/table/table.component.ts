...
export class TableComponent {
  ...
  constructor() {
    type RawJornada = TeamStanding[] & {
      name?: string;
      score_best?: JornadaEntry['score_best'];
      score_worst?: JornadaEntry['score_worst'];
      score_average?: number;
    };
    const raw = this.dataService.getJornadas() as Record<string, RawJornada>;
    const entries: JornadaEntry[] = Object.keys(raw)
      ...
      .map((key) => ({
        ...
        teamsByJornada: ([...raw[key]] as TeamStanding[]).sort(
          (a, b) => a.position_jornada - b.position_jornada,
        ),
